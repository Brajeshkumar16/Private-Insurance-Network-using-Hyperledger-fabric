...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating AceHospitals Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-Ace.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating OakHospitals Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-Oak.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating AkanshaInsurance Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-Akansha.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating LifecareInsurance Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-Lifecare.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating DiagnosticCentre Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-Diagnostic.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/Ace/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating Ace Identities"

    createAce

    infoln "Creating Oak Identities"

    createOak

    infoln "Creating Akansha Identities"

    createAkansha

    infoln "Creating Lifecare Identities"

    createLifecare

    infoln "Creating Diagnostic Identities"
...
# call the script to create the channel, join the peers of Ace and Oak,
...
function networkDown() {
  ...
  # stop org3 containers also in addition to Ace and Oak, in case we were running sample to add org3
  ...
  # # stop org4 containers also in addition to Ace and Oak, in case we were running sample to add org4
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com    #docker_DoctorPeer docker_NursePeer docker_AnchDoctor docker_EndorsStaff docker_AdmissionsPeer docker_OakDoctorPeer docker_OakNursePeer docker_OakAnchDoctor docker_OakEndorsStaff docker_OakAdmissionsPeer docker_AkanshaEndorser docker_AkanshaCommitters docker_AkanshaAnchors docker_LifecareEndorser docker_LifecareCommitters docker_LifecareAnchors docker_DiagnosticEndorser docker_DiagnosticCommitters docker_DiagnosticAnchors
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/Ace/msp organizations/fabric-ca/Ace/tls-cert.pem organizations/fabric-ca/Ace/ca-cert.pem organizations/fabric-ca/Ace/IssuerPublicKey organizations/fabric-ca/Ace/IssuerRevocationPublicKey organizations/fabric-ca/Ace/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/Oak/msp organizations/fabric-ca/Oak/tls-cert.pem organizations/fabric-ca/Oak/ca-cert.pem organizations/fabric-ca/Oak/IssuerPublicKey organizations/fabric-ca/Oak/IssuerRevocationPublicKey organizations/fabric-ca/Oak/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/Akansha/msp organizations/fabric-ca/Akansha/tls-cert.pem organizations/fabric-ca/Akansha/ca-cert.pem organizations/fabric-ca/Akansha/IssuerPublicKey organizations/fabric-ca/Akansha/IssuerRevocationPublicKey organizations/fabric-ca/Akansha/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/Lifecare/msp organizations/fabric-ca/Lifecare/tls-cert.pem organizations/fabric-ca/Lifecare/ca-cert.pem organizations/fabric-ca/Lifecare/IssuerPublicKey organizations/fabric-ca/Lifecare/IssuerRevocationPublicKey organizations/fabric-ca/Lifecare/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/Diagnostic/msp organizations/fabric-ca/Diagnostic/tls-cert.pem organizations/fabric-ca/Diagnostic/ca-cert.pem organizations/fabric-ca/Diagnostic/IssuerPublicKey organizations/fabric-ca/Diagnostic/IssuerRevocationPublicKey organizations/fabric-ca/Diagnostic/fabric-ca-server.db'
